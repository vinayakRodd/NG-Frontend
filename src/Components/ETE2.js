...
function ETE2() {
    ...
    const [ETERelatedPdf,setETERelatedPdf] = useState([])
...
<div className='flex flex-col transition-all   duration-100 ease-in-out  animate-fade-in-slide-up'>
    <div className='text-white text-3xl mt-6 flex justify-center'>
      <span className='text-center'>ETE 2nd Year</span>
    </div>
...
{!isFirstRender.current &&  SearchedSubject.current.value.length !== 0 && ETERelatedPdf.length === 0
  
  ||(ShowSubjectsClicked && (Sem3 || Sem4) && ETERelatedPdf.length === 0) ?
...
{ETERelatedPdf.map((pdf) => (

<div>
{pdf.Modules.length ? 
  <div key={pdf.SubjectNumber} className={`  bg-black  transition-all   duration-100 ease-in-out  animate-fade-in-slide-up ` } >
    <div   className="flex cursor-pointer flex-col gap-2 border-custom-dark  bg-black border-2 rounded-lg shadow-lg p-4 mx-auto w-full max-w-3xl">
